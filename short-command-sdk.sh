...
{    
    ...
    OUTPUT_DIR="${OUTPUT_DIR:-$SDK_DIR/builds}"
    OUTPUT_DIR="$OUTPUT_DIR/$BUILD_VERSION"
    ...
    if [ 1 -eq "$DO_LOAD" ]; then
        _short-command-sdk-source "$OUTPUT_DIR" "$OUTPUT_FILE"
    fi
}

_short-command-sdk-source ()
{
    export SHOCO_INSTALLATION_DIR="$(dirname $1)"
    . "$2"
...
_short-command-sdk-build ()
{
    ...
    sed -i "s/___VERSION___/${BUILD_VERSION}/" $OUTPUT_FILE
